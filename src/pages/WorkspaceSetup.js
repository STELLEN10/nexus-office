...
const PLANS = [
  {
    ...
    features: ["Up to 5 members", "5GB storage", "Basic chat + tasks", "Community support"],
    badge: null,
  },
  {
    id: "pro", name: "Pro", price: "$12", sub: "/user/month",
    features: ["Unlimited members", "100GB storage", "All features + AI", "Priority support"],
    badge: "Most Popular",
  },
];
...
export default function WorkspaceSetup() {
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    try { await createWorkspace({ name: wsName.trim(), plan }); }
      ...
      <div className="setup-card">
        ...
        {step === 0 && (
          <>
            ...
            <p className="setup-sub">Welcome, {profile?.name?.split(" ")[0] || "there"}! Let's set up your team's new home.</p>
            <div className="setup-form">
              <div className="setup-field">
                <label className="setup-label">Workspace name</label>
                <input className="setup-input" type="text"
                  placeholder="Acme Inc, My Team, Studio…"
                  value={wsName} onChange={e => setWsName(e.target.value)}
                  autoFocus onKeyDown={e => e.key === "Enter" && wsName.trim() && setStep(1)}/>
              </div>
              <button className="btn-primary" style={{marginTop: 8}}
                onClick={() => wsName.trim() && setStep(1)} disabled={!wsName.trim()}>
                Continue →
              </button>
            </div>
          </>
        ...
        {step === 1 && (
          <>
            ...
            <p className="setup-sub">Start free — upgrade anytime. No credit card required for Free.</p>
